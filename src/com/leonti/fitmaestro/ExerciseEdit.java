...
public class ExerciseEdit extends Activity {
		...
		if (mRowId != null) {
			...
			mType
					.setChecked(excercise
							.getInt(excercise
									.getColumnIndexOrThrow(ExcercisesDbAdapter.KEY_TYPE)) == 1 ? true
							: false);
	...
	private void saveState() {
		...
		if (mRowId == null) {
			...
			if (title.length() > 0) {
				long id = mDbHelper.createExercise(title, desc, groupId, type,
						maxReps, maxWeight);
				if (id > 0) {
					mRowId = id;
				}
			...
			mDbHelper.updateExercise(mRowId, title, desc, groupId, type,
					maxReps, maxWeight);
		}
	}
}
