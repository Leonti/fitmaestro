...
public class MeasurementsStats extends Activity {
	...
	private void getChart(){
		...
		Cursor maxCursor = measurement.fetchMaxMeasForDates(mMeasurementId, begin, end);
		...
		mMeasLogCursor = measurement.fetchMeasLogEntriesForDates(mMeasurementId, begin, end);
		...
		do{
			...
			if(mMeasLogCursor.getCount() > 0){
				...
				Log.i("CURRENT VALUE:", String.valueOf(mMeasLogCursor.getDouble(mMeasLogCursor.getColumnIndex(FitmaestroDb.KEY_VALUE))));
				try {
					String dbDateUnparsed = mMeasLogCursor.getString(mMeasLogCursor.getColumnIndex(FitmaestroDb.KEY_DATE));
					...
					if(dayToCheck.equals(dbDate)){
						...
						Double value = mMeasLogCursor.getDouble(mMeasLogCursor.getColumnIndex(FitmaestroDb.KEY_VALUE));
